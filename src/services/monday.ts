...
export async function fetchItemMeta(
  ...
  const cvs = (item.column_values || []) as Array<{
    id: string;
    text?: string;
    value?: string;
  }>;
  const emails = EMAIL_COLS.map((id) =>
    cvs.find((c) => c.id === id)?.text?.trim()
  ...
  if (fileCol?.value) {
    try {
      ...
      const assetId = String(
        parsed?.files?.[0]?.assetId ?? parsed?.files?.[0]?.asset_id ?? ""
      );
  ...
  if (!picked) {
    ...
    picked =
      assets.find(
        (a) => a.name?.toLowerCase() === colText.split("/").pop()?.toLowerCase()
      ) ||
